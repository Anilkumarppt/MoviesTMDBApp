...
class PopularMovieListAdapter (): RecyclerView.Adapter<PopularMovieListAdapter.MovieViewHolder>() {
    ...
    private  var popularMovies:List<PopularMovieItem> = emptyList<PopularMovieItem>().toMutableList()

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): MovieViewHolder {
        val mBinding:MoviesItemBinding=MoviesItemBinding.inflate(LayoutInflater.from(parent.context),parent,false)
    ...
    class MovieViewHolder(private val movieItemBinding:MoviesItemBinding):ViewHolder(movieItemBinding.root) {
             fun bindData(popularMovie: PopularMovieItem){
                 Log.d("Adapter", "bindData: ${popularMovie.poster_path}")
                movieItemBinding.popularMovie=popularMovie
